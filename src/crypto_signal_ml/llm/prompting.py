...
def build_tool_system_prompt(system_name: str = "Crypto Signal Copilot") -> str:
    ...
    return (
        ...
        "Add search_knowledge when the user asks why, for context, for sources, for supporting research, or for a broad non-market answer. "
        "For non-market questions, search the RAG knowledge store first; if a general chat model is available and RAG has no match, "
        "it may answer from general knowledge while clearly avoiding current or source-backed claims that were not retrieved. "
